...
# Define the range of (training) patterns and dropout-rates to try
patterns_range = [10, 1000]
dropout_range = [0.5, 1] #[0.8, 1]

# Create local directories for organising text files and figures
results_dir = 'patterns_dropout_random_search_results'
...
while i < max_iterations: # Essentially while True... just a bit safer, in case I die and never quit the program -- but then it's not my problem anyway
    ...
    if (patterns, dropout) in hyperparameters:
        print(f"These hyperparameters already exist, skipping (i={i})!")
        i += 1
        continue

    if (patterns_range[0] == patterns_range[1]) and (dropout_range[0] == dropout_range[1]):
    ...
    layer_defines = [[static_hps['hidden'], act.tanh, static_hps['l2'], dropout],
                     [1, act.sig, static_hps['l2'], dropout]]
    ...
    with open(txt_name, 'a') as f:
        f.write(f'$ {patterns} ; {0} ; {fin_val_loss} ; {val_confusion_matrix.tolist()}\n')

    # Save the error over epochs plot
    if (should_make_plots):
        # Save the error over epochs plot
        plt.savefig(f'{results_dir}/{id_dir}/{seed_dir}/error-over-epochs_patterns-{patterns}_dropout-{dropout}.{img_type}') 

        # Plot the decision boundary
        cs.decision_boundary(val_patterns, val_targets, ann_model)
        plt.savefig(f'{results_dir}/{id_dir}/{seed_dir}/validation-decision-boundary_patterns-{patterns}_dropout-{dropout}.{img_type}')
